...
| P-6  | 初回の style/layout タスク   | 全ページ共通で、読み込み直後にサイト共通 CSS（`styles/*.css` 約 7,000 行）とヘッダー・フッターの style/layout が 1 本の長いタスクになる（4 倍 CPU 減速で約 400ms）。これが FCP より後に終わると TBT に数えられ、同じコードでも CI の TBT が 100〜900ms で揺れる原因になっていた（2026-09-24、PR #34）。ページ別に CSS を分ける、使われていないルールと重い `:has()` を減らす、フッターの写真帯など共通部品を軽くする | High             |
| P-7  | `/posts` の全件 HTML         | 114 記事すべてのカードを 1 枚の HTML（約 380KB）に持ち、絞り込みとページ送りをクライアントでしている。画面外カードは `content-visibility: auto` で後回しにしたが、記事が増えるほど重くなる。ページごとの静的 HTML か、2 ページ目以降を JSON で後読みにする                                                                                                                                                           | Medium（成長時） |
| P-8  | Home の inline SVG（続き）   | P-2 の続き。HOME の地図は訪問範囲に切り抜き、枠外の国を出力しないようにしたが、それでも約 84KB（HTML の約半分）。外部 SVG 化は未着手                                                                                                                                                                                                                                                                                 | Medium           |
| P-9  | Lighthouse の判定            | 2026-09-24 に各ページ 3 回の**中央値**で判定するよう変更（1 回でも超えたら失敗、だと上の揺れで落ちていた）。残り: mobile preset の追加、`PERF_BASELINE`（main の `perf-baseline.json`）を CI に渡して相対回帰を効かせる                                                                                                                                                                                              | Medium           |
| P-10 | 配色のコントラスト           | `--ink-faint`（#8b897e）と `--accent`（#c25a28）が ivory 背景で 4.5:1 にわずかに届かず、全ページのアクセシビリティが 92〜96 に留まる。2 色を少し濃くすれば 96 以上の見込み（全体の色味が変わるのでデザイン判断が要る）                                                                                                                                                                                               | Low              |
...
| 種別           | 内容                                                                                                                                                    | 対応                                                                                                            |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| 巨大ファイル   | `IndexPage.astro`（残り 5 画面、`HomePage` は 2026-09-24 に分割済み）。`PagePage.astro` 280 行に FAQ / Contact 分岐                                     | `ArchivePage` / `DestinationIndexPage` / `CollectionIndexPage` / `SitemapPage` / `GalleryPage` に分割。挙動不変 |
| 二重 URL バグ  | SEO-1                                                                                                                                                   | 即修正                                                                                                          |
