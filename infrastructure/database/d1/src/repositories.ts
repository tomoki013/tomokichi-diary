...
function upsert(db: SqlDatabase, table: string, row: Row, keyColumns: readonly string[]): SqlStatement {
...
export function createRepositories(db: SqlDatabase): Repositories {
  const all = <T>(sql: string, ...values: readonly unknown[]): Promise<Row[]> =>
    db.prepare(sql).bind(...values).all<Row>() as Promise<Row[]>;
  const first = (sql: string, ...values: readonly unknown[]): Promise<Row | null> =>
    db.prepare(sql).bind(...values).first<Row>();
  ...
  const articles: ArticleRepository = {
    ...
    listAll: async () => (await all("SELECT * FROM articles ORDER BY published_at DESC, created_at DESC")).map(articleRow.to),
    save: async (article) => {
  ...
  const revisions: RevisionRepository = {
    ...
    listByArticle: async (articleId) =>
      (await all("SELECT * FROM article_revisions WHERE article_id = ? ORDER BY revision_number DESC", articleId)).map(
        revisionRow.to,
      ),
    save: async (revision) => {
  ...
  const embeds: EmbedRepository = {
    listByRevision: async (revisionId) =>
      (await all("SELECT * FROM article_embeds WHERE revision_id = ?", revisionId)).map(embedRow.to),
  ...
  const locations: LocationRepository = {
    ...
    save: async (location, names) => {
      await db.batch([
        ...
        ...names.map((name) => upsert(db, "location_names", locationNameRow.from(name), ["location_id", "locale"])),
      ]);
    },
  };
  ...
  const media: MediaRepository = {
    ...
    listAll: async () => (await all("SELECT * FROM media_assets ORDER BY created_at DESC")).map(mediaRow.to),
    save: async (asset) => {
      await upsert(db, "media_assets", mediaRow.from(asset), ["id"]).run();
    },
    listForArticle: async (articleId) =>
      (await all("SELECT * FROM article_media WHERE article_id = ? ORDER BY sort_order", articleId)).map(articleMediaRow.to),
    replaceForArticle: async (articleId, list) => {
      await db.batch([
        db.prepare("DELETE FROM article_media WHERE article_id = ?").bind(articleId),
        ...list.map((usage) => upsert(db, "article_media", articleMediaRow.from(usage), ["article_id", "media_id", "role"])),
      ]);
    },
  };

  const taxonomy: TaxonomyRepository = {
    listCategories: async () => (await all("SELECT * FROM categories ORDER BY sort_order, slug")).map(categoryRow.to),
  ...
  const relations: RelationRepository = {
    listArticleLocations: async () => (await all("SELECT * FROM article_locations")).map(articleLocationRow.to),
    listArticlePlaces: async () => (await all("SELECT * FROM article_places")).map(articlePlaceRow.to),
    listArticleCategories: async () => (await all("SELECT * FROM article_categories")).map(articleCategoryRow.to),
    ...
    replaceForArticle: async (articleId, next) => {
      await db.batch([
        ...
        ...next.locations.map((r) => upsert(db, "article_locations", articleLocationRow.from(r), ["article_id", "location_id", "relation"])),
        ...next.places.map((r) => upsert(db, "article_places", articlePlaceRow.from(r), ["article_id", "place_id", "relation"])),
        ...next.categories.map((r) => upsert(db, "article_categories", articleCategoryRow.from(r), ["article_id", "category_id"])),
        ...next.tags.map((r) => upsert(db, "article_tags", articleTagRow.from(r), ["article_id", "tag_id"])),
      ]);
    },
  };
  ...
  return { articles, revisions, embeds, routes, locations, places, media, taxonomy, relations, authors, aiArtifacts };
}
